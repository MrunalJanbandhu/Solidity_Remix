...
contract EscrowMarketplace is AccessManager {
    ...
    error InvalidState(uint256 orderId, OrderState current);
    ...
    constructor(address payable treasuryAddr, address arbitratorAddr) {
        if (treasuryAddr  == address(0)) revert ZeroAddress();
        if (arbitratorAddr == address(0)) revert ZeroAddress();

        treasury   = Treasury(treasuryAddr);
        arbitrator = IArbitrator(arbitratorAddr);
    }

    // Buyer flow
    function createOrder(address seller, bytes32 detailsHash) external returns (uint256 orderId) {
    ...
    function markShipped(uint256 orderId) external {
        ...
        if (o.seller != msg.sender)       revert NotSeller(orderId);
        if (o.state != OrderState.Funded) revert InvalidState(orderId, o.state);
    ...
    function _getOrder(uint256 orderId) internal view returns (Order storage) {
        ...
        if (o.buyer == address(0)) revert OrderNotFound(orderId);
        return o;
    }
